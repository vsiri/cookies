...
class EventBasedAnimationClass(object):
    ...
    def runCommand(self, command):
        self.keyText = ""
        if self.lose:
            if self.timerCounter >= self.lose and command == "yes":
                self.reset()
        elif self.macaron:
            if command == "continue":
                self.macaron += 1
    ...
    def onTimerFired(self):
        ...
        if self.cookieTimer:
            ...
            if self.cookieTimer == 11:
                ...
                self.gameOver("burnt")
        if self.lose and self.timerCounter == self.lose:
    ...
    def redrawAll(self):
        ...
        entered_text = self.canvas.create_text(20,968,text="> " + self.keyText, fill="white", anchor=W, font=('Helvetica', 20))
    ...
    def gameOver(self, string):
        self.lose = self.timerCounter + 25
        if string == "fire":
